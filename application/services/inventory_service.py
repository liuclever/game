...
class InventoryService:
    ...
    def use_item(self, user_id: int, inv_item_id: int, quantity: int = 1) -> dict:
        ...
        SKILL_BOOK_POUCH_ITEM_ID):
            ...
            for _ in range(quantity):
                ...
                for item_cfg in rewards_def.get("items", []) or []:
                    ...
                    if not item_id and "pool" in item_cfg:
                        ...
                        if pool:
                            # 检查是否只选择一次（pick_once标志）
                            if item_cfg.get("pick_once", False):
                                # 只随机选择一个物品ID，然后给予count数量
        ...
        elif item_template.id in (7101, 7102, 7103, 7104, 7105, 7106):  # 战灵灵石
            # 检查等级限制（35级才能使用战灵系统）
            if not self.player_repo:
                raise InventoryError("系统错误：PlayerRepo 未配置")
            player = self.player_repo.get_by_id(user_id)
            if not player:
                raise InventoryError("玩家不存在")
            if (player.level or 0) < 35:
                raise InventoryError("战灵系统需要35级才能解锁，战灵灵石暂时无法使用")
            ...
            element_key = stone_element_map.get(item_template.id)
            if not element_key:
                raise InventoryError("未知的战灵灵石类型")
            ...
            spirit_service = SpiritService(
                ...
                player_repo=self.player_repo,
                tower_state_repo=None,
                inventory_service=self,
            )
            
            try:
                created_spirits = spirit_service.open_stone(user_id, element_key, quantity)
                element_name = element_name_map.get(element_key, element_key)
                message = f"成功开启{item_template.name}×{quantity}，获得{len(created_spirits)}个{element_name}灵"
                rewards[f"{element_name}灵"] = len(created_spirits)
            except Exception as e:
                raise InventoryError(f"开启战灵灵石失败：{str(e)}")
        else:
