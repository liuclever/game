...
运行方式（项目根目录）：
    ...
    python -m pytest tests/battle_filed/Feihe_filed/test_battlefield_http_e2e.py -vv -s

前置条件：
    - Flask 应用已启动: python -m interfaces.web_api.app
    - 数据库中存在测试账号（默认 test_user / test123）且满足飞鹤战场等级(40-100)
...
TEST_USERNAME = "test1"  # 飞鹤战场测试玩家（需先运行 create_31_crane_battlefield_players.py）
TEST_PASSWORD = "123456"
BATTLEFIELD_TYPE = "crane"


class TestBattlefieldHttpE2E:
    """古战场全链路 HTTP 端到端测试"""
