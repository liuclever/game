...
def challenge_arena():
    ...
    if not challenger_beasts or not champion_beasts:
        ...
        # 在battle_data中添加连胜信息，方便动态显示
        if challenger_wins:
            # 挑战成功，新擂主连胜为1（首次抢占）
            battle_data['new_consecutive_wins'] = 1
        else:
            # 守擂成功，连胜+1
            battle_data['new_consecutive_wins'] = consecutive_wins + 1
            battle_data['old_consecutive_wins'] = consecutive_wins
