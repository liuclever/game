...
def get_friend_requests():
    ...
    mail_service = MailService()
    result = mail_service.get_friend_requests(
        user_id=user_id,
        page=page,
        page_size=page_size
    )
...
@mail_bp.get('/friend-request/setting')
def get_allow_stranger_setting():
    """获取是否允许陌生人添加好友的设置"""
    user_id = get_current_user_id()
    if not user_id:
        return jsonify({"ok": False, "error": "请先登录"}), 401
    ...
    mail_service = MailService()
    allow = mail_service.get_allow_stranger_setting(user_id)
    return jsonify({"ok": True, "allow_stranger_add_friend": allow})


@mail_bp.post('/friend-request/setting')
def set_allow_stranger_setting():
    """设置是否允许陌生人添加好友"""
    user_id = get_current_user_id()
    if not user_id:
        return jsonify({"ok": False, "error": "请先登录"}), 401
    ...
    mail_service = MailService()
    result = mail_service.set_allow_stranger_setting(user_id, allow)
