...
def get_player_profile():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"获取玩家信息失败: {str(e)}"}), 500
    ...
    try:
        ...
        # 如果计算失败，使用原始数据
        import traceback
        print(f"计算玩家 {target_id} 幻兽属性失败: {e}")
        print(traceback.format_exc())
        # 使用之前已经构建的beasts_data（如果计算失败，至少返回基本信息）
        if not beasts_data:
            beasts_data = [
                {
                    "id": b.id,
                    ...
                    "realm": b.realm,
                    "level": b.level,
                    "combat_power": b.combat_power or 0,
                }
                for b in beasts
            ]
    ...
    try:
        ...
        # 如果获取动态失败，返回空列表（不影响其他信息显示）
        import traceback
        print(f"获取玩家 {target_id} 动态失败: {e}")
        print(traceback.format_exc())
        dynamics = []
    ...
    try:
        ...
        import traceback
        print(f"获取玩家 {target_id} 联盟信息失败: {e}")
        print(traceback.format_exc())
        pass
    ...
    try:
        ...
        import traceback
        print(f"构建玩家 {target_id} 响应数据失败: {e}")
        print(traceback.format_exc())
        return jsonify({"ok": False, "error": f"构建响应数据失败: {str(e)}"}), 500
...
def spar_battle():
    ...
    # 保存切磋记录到数据库
    try:
        from infrastructure.db.connection import execute_insert
        import json
        battle_json = json.dumps(battle_data, ensure_ascii=False)
        execute_insert(
            """INSERT INTO spar_battle_log 
               (attacker_id, attacker_name, defender_id, defender_name, is_attacker_win, battle_data)
               VALUES (%s, %s, %s, %s, %s, %s)""",
            (user_id, attacker.nickname, target_id, defender.nickname, 1 if is_win else 0, battle_json)
        )
    except Exception as e:
        # 如果表不存在或其他错误，记录日志但不影响功能
        print(f"保存切磋记录失败: {e}")
