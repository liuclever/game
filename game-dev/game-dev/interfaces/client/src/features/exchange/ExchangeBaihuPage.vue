...
const fetchExchangeInfo = async () => {
  loading.value = true
  try {
    const res = await http.get('/exchange/beast/baihu/status')
    if (res.data?.ok) {
      exchangeInfo.value = {
...
const handleRedeem = async () => {
  ...
  try {
    const res = await http.post('/exchange/beast/baihu')
    if (res.data?.ok) {
...
<template>
  <div class="exchange-detail-page">
    <div class="section title">【兑换】神兽[神·白虎]</div>

    <div class="section">
      建议40-59级召唤师兑换：
    </div>
    <div class="section indent">
      集齐6块[神·逆鳞]，可获得中级神兽[神·白虎]，40级可携带！
    </div>
    <div class="section indent">
      神·白虎是中级幻兽的王者，且4个技能均为高级技能！
    </div>
    <div class="section indent">
