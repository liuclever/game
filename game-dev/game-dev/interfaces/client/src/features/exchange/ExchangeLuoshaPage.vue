...
const fetchExchangeInfo = async () => {
  loading.value = true
  try {
    const res = await http.get('/exchange/beast/luosha/status')
    if (res.data?.ok) {
      exchangeInfo.value = {
...
const handleRedeem = async () => {
  ...
  if (!exchangeInfo.value.canExchange) {
    toast.error('神·逆鳞不足，需要${exchangeInfo.value.required}块')
    return
  }

  isRedeeming.value = true
  try {
    const res = await http.post('/exchange/beast/luosha')
    if (res.data?.ok) {
      toast.info(res.data.message || '兑换成功')
...
<template>
  <div class="exchange-detail-page">
    <div class="section title">【兑换】神兽[神·罗刹]</div>

    <div class="section">建议35-59级召唤师兑换：</div>
    <div class="section indent">
      集齐6块[神·逆鳞]，可获得中级神兽[神·罗刹]，35级可携带！
    </div>
    <div class="section indent">
      神·罗刹是中级幻兽的王者，且4个技能均为高级技能！
    </div>
    <div class="section indent">
      提示：“远古钛金宝箱”、“远古秘银宝箱”开启有几率获得[神·逆鳞]
    </div>

    <div class="section">[指引]: 集齐6块[神·逆鳞]</div>
    <div class="section">
      [当前数量]: {{ exchangeInfo.currentNilin }}块神·逆鳞
      <span v-if="loading" class="gray">（加载中…）</span>
    </div>
    <div class="section">[奖励]: 神兽[神·罗刹]</div>
    <div class="section">
      [状态]:
      <a
        class="link"
