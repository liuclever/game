...
const fetchExchangeInfo = async () => {
  loading.value = true
  try {
    const res = await http.get('/exchange/beast/xuanwu/status')
    if (res.data?.ok) {
      exchangeInfo.value = {
...
const handleRedeem = async () => {
  ...
  try {
    const res = await http.post('/exchange/beast/xuanwu')
    if (res.data?.ok) {
...
<template>
  <div class="exchange-detail-page">
    <div class="section title">【兑换】神兽[神·玄武]</div>

    <div class="section">
      建议全体召唤师兑换：
    </div>
    <div class="section indent">
      集齐10块[神·逆鳞]，可获得顶级神兽[神·玄武]，60级可携带！
    </div>
    <div class="section indent">
      神·玄武是召唤大陆最顶级的幻兽，且4个技能均为高级技能！
    </div>
    <div class="section indent">
