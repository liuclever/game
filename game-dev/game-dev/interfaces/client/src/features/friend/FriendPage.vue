...
const spar = (f) => {
  toast.error(`切磋：${f.nickname}`)
}

const infuse = (f) => {
  toast.error(`灌注：${f.nickname}`)
}
...
const openChat = (friend) => {
  router.push({
    path: '/mail/chat',
    query: {
      target_id: friend.id || friend.user_id,
  ...
  <div class="friend-page">
    ...
    <template v-if="activeTab === TAB_LEVEL">
      <div v-for="(f, idx) in pageItems" :key="f.id" class="section item">
        ...
        <a class="link name" :class="{ red: f.highlight }" @click="openChat(f)">{{ f.nickname }}</a>
