...
const doHuaxian = async (beast) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.info('【${name}】已化仙，获得经验 ${expAdded}！')
      await loadData()
    } else {
      toast.error(res.data.error || '化仙失败')
    }
  } catch (err) {
...
const startFormation = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.error(res.data.error || '开启失败')
    }
  } catch (err) {
    toast.error('网络错误，请稍后重试')
  } finally {
...
const harvestExp = async () => {
  ...
  if (formation.value.pendingExp > 0) {
    toast.info('已收获 ${formation.value.pendingExp} 经验！')
  } else {
    toast.info('暂无可收获的经验')
  }
}
...
const useHuaxianDan = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      toast.info(res.data.message || '使用成功')
      await loadData()
    } else {
      toast.error(res.data.error || '使用失败')
    }
  } catch (err) {
    toast.error('网络错误，请稍后重试')
  } finally {
