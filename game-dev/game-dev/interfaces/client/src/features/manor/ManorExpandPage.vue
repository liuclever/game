...
const handleExpand = async () => {
  if (!isSpecialLand.value && nextLandIndex.value >= 10) {
    toast.error('普通土地最多10块，无法继续扩建');
    return;
  }
  if (!levelSatisfied.value) {
    toast.error(`等级不足，需要${toChineseStar(requiredStar.value)}星召唤师（${requiredLevel.value}级）`);
    return;
  }
  if (!itemSatisfied.value) {
    toast.error(`道具不足，需要庄园建造手册×${requiredManuals.value}`);
    return;
  }
  if (isSpecialLand.value && !vipSatisfied.value) {
    toast.error(`VIP等级不足，需要VIP ${requiredVip.value}`);
    return;
  }
