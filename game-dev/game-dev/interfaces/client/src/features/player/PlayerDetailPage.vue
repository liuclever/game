...
const sendMessage = () => {
  ...
  router.push({
    ...
    query: {
      ...
      name: player.value.nickname || `玩家${player.value.id}`
    }
  })
}
...
// 加为好友
const addFriend = () => {
  toast.info('添加好友功能待实现')
}

// 拉黑
const blockPlayer = () => {
  toast.info('拉黑玩家功能待实现')
}
...
const doInfuse = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.error(res.data.error || '灌注失败')
    }
  } catch (e) {
    console.error('灌注失败', e)
    toast.error(e?.response?.data?.error || '灌注失败')
  } finally {
...
const challenge = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.error(res.data.error || '切磋失败')
    }
  } catch (e) {
    console.error('切磋失败', e)
    toast.error(e?.response?.data?.error || '切磋失败')
  } finally {
...
// 查看天赋
const viewTalent = () => {
  toast.info('查看天赋功能待实现')
}

// 查看联盟
const viewAlliance = () => {
  toast.info('查看联盟功能待实现')
}
