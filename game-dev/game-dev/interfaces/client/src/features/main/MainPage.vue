...
const { showSuccess, showError } = useOperationResult()
...
const harvestCultivation = async () => {
  try {
    ...
    showError({
      message: e?.response?.data?.error || '领取奖励失败',
      backPath: '/',
      backText: '返回首页'
    })
  }
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      if (res.data.rewards) {
        ...
        let detail = `获得声望: ${rewards.prestige}\n获得强化石: ${rewards.spirit_stones}`
    ...
    showError({
      message: e.response?.data?.error || '终止修行失败',
      backPath: '/',
      backText: '返回首页'
    })
  } finally {
...
const doLevelup = async () => {
  try {
    ...
    showError({
      message: e?.response?.data?.error || '晋级失败',
      backPath: '/',
      backText: '返回首页'
    })
  }
