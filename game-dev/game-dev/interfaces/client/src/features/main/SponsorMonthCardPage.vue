...
const handlePurchase = async () => {
  ...
  try {
    ...
    if (res.data?.ok) {
      toast.success(`购买成功！已发放 ${res.data.data?.immediate_reward ?? 1000} 元宝。`)
      await fetchStatus()
    } else {
      throw new Error(res.data?.error || '购买失败')
    }
  } catch (err) {
    toast.error(err?.response?.data?.error || err.message || '购买失败，请稍后再试')
  } finally {
...
const handleClaim = async () => {
  ...
  try {
    ...
    if (res.data?.ok) {
      toast.success(`领取成功！获得 ${res.data.data?.reward ?? 200} 元宝。`)
      await fetchStatus()
    } else {
      throw new Error(res.data?.error || '领取失败')
    }
  } catch (err) {
    toast.error(err?.response?.data?.error || err.message || '领取失败，请稍后再试')
  } finally {
