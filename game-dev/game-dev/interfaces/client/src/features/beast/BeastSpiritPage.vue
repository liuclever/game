...
const unequipSpirit = async (elementKey) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.success('卸下成功，战灵已存入灵件室')
    } else {
      toast.error(res.data.error || '卸下失败')
    }
  } catch (err) {
...
const showIntro = () => {
  toast.info('战灵简介：战灵可以提升幻兽的百分比属性加成，每只幻兽可装备6个元素位置的战灵（土、火、水、木、金、神）。战灵需要与幻兽种族匹配才能装备。')
}
