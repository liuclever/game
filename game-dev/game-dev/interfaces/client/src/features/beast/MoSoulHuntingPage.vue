...
const doHunting = async (hunter) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.error(res.data.error || '猎魂失败')
    }
  } catch (err) {
...
const doOneKeyHunting = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.error(res.data.error || '一键猎魂失败')
    }
  } catch (err) {
    toast.error('网络错误，请稍后重试')
...
<template>
  <div class="hunting-page">
    ...
    <div class="section title-section">
      【魔魂-猎魂】 <a class="link" @click="() => window.toast.info('猎魂简介：花费铜钱让猎魂师帮你捕捉魔魂，不同猎魂师获得高品质魔魂的概率不同。')">简介</a>
    </div>
