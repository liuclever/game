...
const sellSpirit = async (spirit) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.success('出售成功，获得 ${res.data.spiritPower || 0} 灵力')
    } else {
      toast.error(res.data.error || '出售失败')
    }
  } catch (err) {
    toast.error('网络错误，请稍后重试')
...
const sellAllUnlocked = async () => {
  ...
  // 重新加载数据
  await loadData()
  toast.success('成功出售 ${successCount} 个战灵，获得 ${totalPower} 灵力')
}
