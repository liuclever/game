...
const fetchMembers = async (sort = sortType.value) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      toast.error(res.data.error || '获取成员信息失败')
    }
  } catch (e) {
    toast.error(e.response?.data?.error || '获取成员信息失败')
  } finally {
    loading.value = false
...
const assignRole = async (member) => {
  ...
  try {
    ...
    if (res.data.ok) {
      toast.success('职位调整成功')
      fetchMembers()
    } else {
      toast.error(res.data.error || '调整失败')
    }
  } catch (e) {
    toast.error(e.response?.data?.error || '调整失败')
  }
}
