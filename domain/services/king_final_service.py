...
def run_final_stage(stage):
    ...
    # 特殊处理：champion阶段是从'2'强中决出冠军
    if stage == 'champion':
        print(f"[召唤之王] 开始执行决赛（冠军赛）...")
        
        # 获取进入决赛的2名选手
        finalists = execute_query(
            """SELECT user_id FROM king_final_stage 
               WHERE season = %s AND stage = '2' AND is_winner = 1
               ORDER BY id ASC
               LIMIT 2""",
            (season,)
        )
        ...
        if len(finalists) != 2:
            print(f"[召唤之王] 决赛选手不足2人，无法进行决赛")
            return
        
        player1_id = finalists[0]['user_id']
        player2_id = finalists[1]['user_id']
        
        print(f"[召唤之王] 决赛: {player1_id} vs {player2_id}")
        
        # 执行决赛战斗
        winner_id = execute_battle(player1_id, player2_id, season, 'champion', 1)
        
        print(f"[召唤之王] 冠军诞生: {winner_id}")
        
        # 记录决赛结果
        execute_update(
            """INSERT INTO king_final_stage 
               (season, user_id, stage, is_winner, opponent_id, match_id, battle_time, created_at) 
               VALUES (%s, %s, 'champion', 1, %s, 1, NOW(), NOW())""",
            (season, winner_id, player2_id if winner_id == player1_id else player1_id)
        )
        ...
        # 发放冠军奖励
        send_stage_reward(winner_id, 'champion', season)
        
        # 发放亚军奖励
        runner_up_id = player2_id if winner_id == player1_id else player1_id
        send_stage_reward(runner_up_id, '2', season)
        
        print(f"[召唤之王] 决赛结束，冠军: {winner_id}")
        return
    ...
    next_stage_map = {
        ...
        '8': '4',
        '4': '2',
        '2': 'final'  # 2强赛后不再插入记录，等待champion阶段
    }
