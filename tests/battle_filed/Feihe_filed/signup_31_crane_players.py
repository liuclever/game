...
运行方式：
    python tests/battle_filed/Feihe_filed/signup_31_crane_players.py

功能：
    - 为 create_31_crane_battlefield_players.py 创建的31个玩家（user_id 4000-4030）报名飞鹤战场
    - 清空这些玩家当日的旧报名记录后重新报名
    - 验证所有玩家都有出战幻兽且等级在40-100范围内
...
# 测试玩家ID范围（与 create_31_crane_battlefield_players.py 保持一致）
START_USER_ID = 4000
END_USER_ID = 4030
NUM_PLAYERS = 31
...
def validate_test_players():
    ...
    if len(players) < NUM_PLAYERS:
        ...
        print("   请先运行: python tests/battle_filed/Feihe_filed/create_31_crane_battlefield_players.py")
        return None
...
def main():
    ...
    try:
        ...
        if final_count >= NUM_PLAYERS:
            ...
            print("下一步可以：")
            print("  1. 运行战场淘汰赛：")
            print("     python scripts/run_daily_battlefield.py --type crane")
            print("  2. 或通过API触发战斗")
        else:
