...
def _run_king_32_stage():
    """每周五12:00执行：32强赛"""
    logger.info("[Scheduler] 开始执行召唤之王32强赛")
    try:
        run_final_stage('32')
        logger.info("[Scheduler] 召唤之王32强赛完成")
    except Exception as e:
        logger.exception(f"[Scheduler] 召唤之王32强赛失败: {e}")


def _run_king_16_stage():
    """每周五13:00执行：16强赛"""
    logger.info("[Scheduler] 开始执行召唤之王16强赛")
    try:
        run_final_stage('16')
        logger.info("[Scheduler] 召唤之王16强赛完成")
    except Exception as e:
        logger.exception(f"[Scheduler] 召唤之王16强赛失败: {e}")


def _run_king_8_stage():
    """每周五14:00执行：8强赛"""
    logger.info("[Scheduler] 开始执行召唤之王8强赛")
    try:
        run_final_stage('8')
        logger.info("[Scheduler] 召唤之王8强赛完成")
    except Exception as e:
        logger.exception(f"[Scheduler] 召唤之王8强赛失败: {e}")


def _run_king_4_stage():
    """每周五15:00执行：4强赛"""
    logger.info("[Scheduler] 开始执行召唤之王4强赛")
    try:
        run_final_stage('4')
        logger.info("[Scheduler] 召唤之王4强赛完成")
    except Exception as e:
        logger.exception(f"[Scheduler] 召唤之王4强赛失败: {e}")


def _run_king_2_stage():
    """每周五16:00执行：决赛"""
    logger.info("[Scheduler] 开始执行召唤之王决赛")
    try:
        run_final_stage('2')
        logger.info("[Scheduler] 召唤之王决赛完成，冠军诞生！")
    except Exception as e:
        logger.exception(f"[Scheduler] 召唤之王决赛失败: {e}")
