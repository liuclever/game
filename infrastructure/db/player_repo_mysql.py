...
class MySQLPlayerRepo(IPlayerRepo):
    ...
    def get_by_id(self, user_id: int) -> Optional[Player]:
        """获取玩家信息"""
        sql = """
            SELECT user_id, username, nickname, level, exp, gold, silver_diamond, yuanbao, dice, enhancement_stone, 
                   energy, prestige, crystal_tower, charm, location, vip_level, vip_exp, last_energy_recovery_time,
                   last_signin_date, consecutive_signin_days,
                   created_at, updated_at, cultivation_start_time, cultivation_duration, cultivation_area, cultivation_dungeon
        ...
        if rows:
            ...
            return Player(
                ...
                copper=row.get('gold') or 0,  # 使用 gold 字段代替 copper
                ...
                consecutive_signin_days=row.get('consecutive_signin_days') or 0,
                created_at=row.get('created_at'),
                updated_at=row.get('updated_at'),
                cultivation_start_time=row.get('cultivation_start_time'),
                cultivation_duration=row.get('cultivation_duration') or 0,
    ...
    def save(self, player: Player) -> None:
        ...
        execute_update(sql, (
            ...
            player.last_signin_date, player.consecutive_signin_days,
            player.user_id
        ))
    ...
    def _ensure_floors_exist(self, floors: List[int]) -> None:
        ...
        for f in floors:
            try:
                ...
                # ignore errors to avoid breaking read paths
                pass
