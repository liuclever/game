...
const spar = (f) => {
  console.error(`切磋：${f.nickname}`)
}

const infuse = (f) => {
  console.error(`灌注：${f.nickname}`)
}
...
const unblockUser = async (user) => {
  ...
  try {
    ...
    if (res.data.ok) {
      console.error(res.data.message || '已取消拉黑')
      loadBlacklist()
    } else {
      console.error(res.data.error || '取消拉黑失败')
    }
  } catch (e) {
    console.error('取消拉黑失败', e)
    console.error(e?.response?.data?.error || '取消拉黑失败')
  } finally {
...
<template>
  <div class="friend-page">
    ...
    <div class="section nav">
      ...
      <a class="link" @click="handleLink('地图')">地图</a>. <a class="link" @click="handleLink('天赋')">天赋</a>. <a class="link" @click="handleLink('化仙')">化仙</a>
    </div>
    <div class="section nav">
      <a class="link" @click="handleLink('切磋')">切磋</a>. <a class="link" @click="handleLink('闯塔')">闯塔</a>.
    ...
    <div class="section nav">
      <a class="link" @click="handleLink('兑换')">兑换</a>. <a class="link" @click="handleLink('签到')">签到</a>.
