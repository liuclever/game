...
const doHuaxian = async (beast) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(`【${name}】已化仙，获得经验 ${expAdded}！`)
      await loadData()
    } else {
      console.error(res.data.error || '化仙失败')
    }
  } catch (err) {
...
const startFormation = async () => {
  ...
  if (!canStartFormation.value) {
    ...
    console.error(`材料不足：需要${missingNames}各1个`)
    return
  }
  ...
  try {
    ...
    if (res.data.ok) {
      console.error('化仙阵已开启！')
      await loadData()
    } else {
      console.error(res.data.error || '开启失败')
    }
  } catch (err) {
    console.error('网络错误，请稍后重试')
  } finally {
...
const harvestExp = async () => {
  ...
  if (formation.value.pendingExp > 0) {
    console.error(`已收获 ${formation.value.pendingExp} 经验！`)
  } else {
    console.error('暂无可收获的经验')
  }
}
...
const useHuaxianDan = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      console.error(res.data.message || '使用成功')
      await loadData()
    } else {
      console.error(res.data.error || '使用失败')
    }
  } catch (err) {
    console.error('网络错误，请稍后重试')
  } finally {
