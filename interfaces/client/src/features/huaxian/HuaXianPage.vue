...
const doHuaxian = async (beast) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      showMessage(`【${name}】已化仙，获得经验 ${expAdded}！`, 'success')
      await loadData()
    } else {
      showMessage(res.data.error || '化仙失败', 'error')
    }
  } catch (err) {
...
const startFormation = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      showMessage('化仙阵已开启！', 'success')
      await loadData()
    } else {
      showMessage(res.data.error || '开启失败', 'error')
    }
  } catch (err) {
    showMessage('网络错误，请稍后重试', 'error')
  } finally {
...
const harvestExp = async () => {
  ...
  if (formation.value.pendingExp > 0) {
    showMessage(`已收获 ${formation.value.pendingExp} 经验！`, 'success')
  } else {
    showMessage('暂无可收获的经验', 'info')
  }
}
...
const useHuaxianDan = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      showMessage(res.data.message || '使用成功', 'success')
      await loadData()
    } else {
...
// 查看简介
const viewIntro = () => {
  showMessage('化仙池简介：\n\n化仙池可以将幻兽转化为经验，用于提升其他幻兽的等级。\n\n战斗队中的幻兽和等级未满15级的幻兽无法进行化仙。', 'error')
}
