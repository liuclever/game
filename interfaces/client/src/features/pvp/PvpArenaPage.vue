...
// 查看今日连胜榜
const viewTodayRank = () => {
  showMessage('今日连胜榜', 'info')
}

// 查看历届连胜王
const viewHistoryKings = () => {
  showMessage('历届连胜王', 'info')
}
...
const challenge = (opponent) => {
  showMessage(`切磋 ${opponent.name}`, 'info')
}

// 手动刷新
const manualRefresh = () => {
  showMessage('刷新对手列表', 'info')
}

// 立即刷新(消耗元宝)
const instantRefresh = () => {
  showMessage('消耗50元宝立即刷新', 'info')
}
