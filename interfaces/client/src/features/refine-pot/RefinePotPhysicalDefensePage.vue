...
const cost = aptitudeCosts.physicalDefense
const aptitudeKey = 'physical_def_aptitude'
...
const aptitudeWarning = computed(() => {
  if (!isAptitudeValid.value) return '副幻兽物防资质需高于主幻兽'
  return ''
})
...
const handleStartRefine = async () => {
  ...
  try {
    ...
    if (!res.data?.ok) {
      console.error(res.data?.error || '炼妖失败')
      return
    }
    const { data } = res.data
    console.error(`成功！主幻兽资质 +${data?.delta ?? 0}，新值 ${data?.after ?? '未知'}`)
    await reload()
  } catch (err) {
...
<template>
  <RefinePotAptitudeTemplate
    title="【炼妖壶-物防资质炼妖】"
    description="对主幻兽进行物防资质炼化，副幻兽物防资质越高效果越好"
