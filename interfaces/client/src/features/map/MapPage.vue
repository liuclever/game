...
const moveToCity = async (cityName) => {
  if (moving.value) {
    showMessage(`移动中，无法再次移动（剩余${remainingSeconds.value}秒）`, 'error')
    return
  }
  const city = CITIES.find(c => c.name === cityName)
  if (city && playerLevel.value < city.min_level) {
    showMessage(`等级不足，需要${city.min_level}级才能前往${cityName}`, 'error')
    return
  }
  try {
...
const teleportToCity = (cityName) => {
  if (moving.value) {
    showMessage(`移动中，无法传送（剩余${remainingSeconds.value}秒）`, 'error')
    return
  }
  router.push({
    path: '/map/teleport',
...
const enterDungeon = (dungeon) => {
  if (moving.value) {
    showMessage(`移动中，无法挑战副本（剩余${remainingSeconds.value}秒）`, 'error')
    return
  }
...
const resetDungeon = async (dungeon) => {
  if (moving.value) {
    showMessage(`移动中，无法重置副本（剩余${remainingSeconds.value}秒）`, 'error')
    return
  }
  // 已移除确认提示
  try {
    ...
    if (res.data.ok) {
      showMessage(res.data.message || '重置成功', 'success')
    } else {
      showMessage(res.data.error || '重置失败', 'error')
    }
  } catch (e) {
