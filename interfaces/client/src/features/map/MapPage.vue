...
const moveToCity = async (cityName) => {
  if (moving.value) {
    console.error(`移动中，无法再次移动（剩余${remainingSeconds.value}秒）`)
    return
  }
  const city = CITIES.find(c => c.name === cityName)
  if (city && playerLevel.value < city.min_level) {
    console.error(`等级不足，需要${city.min_level}级才能前往${cityName}`)
    return
  }
  try {
...
const teleportToCity = (cityName) => {
  if (moving.value) {
    console.error(`移动中，无法传送（剩余${remainingSeconds.value}秒）`)
    return
  }
  router.push({
    path: '/map/teleport',
...
const enterDungeon = (dungeon) => {
  if (moving.value) {
    console.error(`移动中，无法挑战副本（剩余${remainingSeconds.value}秒）`)
    return
  }
...
const resetDungeon = async (dungeon) => {
  if (moving.value) {
    console.error(`移动中，无法重置副本（剩余${remainingSeconds.value}秒）`)
    return
  }
  ...
  try {
    ...
    if (res.data.ok) {
      console.error(res.data.message || '重置成功')
    } else {
      console.error(res.data.error || '重置失败')
    }
  } catch (e) {
