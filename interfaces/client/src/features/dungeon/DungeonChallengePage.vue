...
const handleOpenLoot = async (costType = 'energy') => {
  ...
  try {
    ...
    if (data.ok) {
      ...
      console.error(data.error || '开启战利品失败')
    }
  } catch (e) {
...
const handleOpenChest = async (costType = 'energy') => {
  ...
  try {
    ...
    if (data.ok) {
      ...
      console.error(`开启${pendingChest.value.name}成功！${doubleText}\n获得: ${rewardText}`)
      pendingChest.value = null
      await fetchDungeonProgress()
    } else {
      console.error(data.error || '开启失败')
    }
  } catch (e) {
...
const handleOpenChestInModal = async (costType = 'energy') => {
  ...
  try {
    ...
    if (data.ok) {
      const rewardText = data.rewards.map(r => `${r.name} x${r.amount}`).join(', ')
      const doubleText = data.is_double ? '（双倍奖励）' : ''
      console.error(`开启${chestName}成功！${doubleText}\n获得: ${rewardText}`)
      closeAdvanceModal()
    } else {
      console.error(data.error || '开启失败')
    }
...
const handleRPS = async (choice) => {
  ...
  try {
    ...
    const data = await res.json()
    if (data.ok) {
      console.error(`你出了 ${data.player_choice}，对方出了 ${data.bot_choice}\n结果: ${data.message}\n${data.result === 'win' ? '前进' : (data.result === 'lose' ? '后退' : '原地不动')} ${data.change} 层`)
      pendingEvent.value = null
      await fetchDungeonProgress()
    } else {
      console.error(data.error || '操作失败')
