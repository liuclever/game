...
const addFriend = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      console.error(res.data.message || '好友请求已发送')
    } else {
...
const doInfuse = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '灌注失败')
    }
  } catch (e) {
    console.error('灌注失败', e)
    console.error(e?.response?.data?.error || '灌注失败')
  } finally {
...
const challenge = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '切磋失败')
    }
  } catch (e) {
    console.error('切磋失败', e)
    console.error(e?.response?.data?.error || '切磋失败')
  } finally {
