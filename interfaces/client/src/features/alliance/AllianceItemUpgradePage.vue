...
const fetchInfo = async () => {
  ...
  successMsg.value = ''
  try {
    const res = await http.get('/alliance/item-storage/upgrade-info')
    if (res.data.ok) {
      alliance.value = res.data.alliance || null
      info.value = res.data.itemStorage || null
    } else {
      errorMsg.value = res.data.error || '获取寄存仓库信息失败'
    }
  } catch (err) {
    console.error('获取寄存仓库信息失败', err)
...
const confirmUpgrade = async () => {
  ...
  successMsg.value = ''
  try {
    const res = await http.post('/alliance/item-storage/upgrade')
    if (res.data.ok) {
...
<template>
  <div class="item-upgrade-page">
    ...
    <template v-else>
      ...
      <template v-else>
        <div class="line">寄存仓库已达到最高等级或暂未开放更高级。</div>
      </template>
