...
const createRoom = async () => {
  ...
  try {
    ...
    if (!res.data?.ok) {
      showMessage(res.data?.error || '创建修行房间失败', 'error')
    } else {
      await fetchTrainingInfo()
    }
  } catch (err) {
    showMessage(err.response?.data?.error || '创建修行房间失败', 'error')
  } finally {
    saving.value = false
  }
}
...
const joinRoom = async (roomId) => {
  ...
  try {
    ...
    if (!res.data?.ok) {
      showMessage(res.data?.error || '加入失败', 'error')
    } else {
      await fetchTrainingInfo()
    }
  } catch (err) {
    showMessage(err.response?.data?.error || '加入失败', 'error')
  } finally {
...
const claimReward = async (participantId) => {
  ...
  try {
    ...
    if (!res.data?.ok) {
      ...
      showMessage(`成功领取焚火晶 ${res.data.reward} 个`, 'success')
      await fetchTrainingInfo()
    }
  } catch (err) {
    showMessage(err.response?.data?.error || '领取失败', 'error')
  } finally {
