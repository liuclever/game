...
const goToCreateRoom = () => {
  if (!canCreateRoom.value) {
    if (!info.value.hasFireOre) {
      console.error('需要火能原石才能进行修行，请先领取火能原石')
    }
    return
  }
...
const joinRoom = async (roomId) => {
  ...
  try {
    ...
    console.error(err.response?.data?.error || '加入失败')
  } finally {
    saving.value = false
  }
}
...
const endTraining = async (roomId) => {
  ...
  try {
    ...
    if (!res.data?.ok) {
      console.error(res.data?.error || '结束修行失败')
    } else {
      console.error(res.data?.message || '修行已结束')
      await fetchTrainingInfo()
    }
  } catch (err) {
    console.error(err.response?.data?.error || '结束修行失败')
  } finally {
    saving.value = false
  }
}
...
const claimReward = async (participantId) => {
  ...
  try {
    ...
    if (!res.data?.ok) {
      ...
      console.error(`成功领取焚火晶 ${res.data.reward} 个（焚火炉${res.data.furnaceLevel}级）`)
      await fetchTrainingInfo()
    }
  } catch (err) {
    console.error(err.response?.data?.error || '领取失败')
  } finally {
    saving.value = false
  }
}
