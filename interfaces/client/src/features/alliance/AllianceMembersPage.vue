...
const fetchMembers = async (sort = sortType.value) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      showMessage(res.data.error || '获取成员信息失败', 'error')
    }
  } catch (e) {
    showMessage(e.response?.data?.error || '获取成员信息失败', 'error')
  } finally {
    loading.value = false
...
const assignRole = async (member) => {
  ...
  if (Number.isNaN(selectedRole)) {
    showMessage('请选择有效职位', 'info')
    return
  }
  if (selectedRole === member.role) {
    showMessage('该成员已是此职位', 'success')
    return
  }
  try {
    ...
    if (res.data.ok) {
      showMessage('职位调整成功', 'success')
      fetchMembers()
    } else {
      showMessage(res.data.error || '调整失败', 'error')
    }
  } catch (e) {
    showMessage(e.response?.data?.error || '调整失败', 'error')
  }
}
...
const kickMember = async (member) => {
  ...
  try {
    ...
    if (res.data.ok) {
      showMessage('已踢出该成员', 'success')
      fetchMembers()
    } else {
      showMessage(res.data.error || '操作失败', 'error')
    }
  } catch (e) {
