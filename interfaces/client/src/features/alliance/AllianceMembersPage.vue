...
const fetchMembers = async (sort = sortType.value) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '获取成员信息失败')
    }
  } catch (e) {
    console.error(e.response?.data?.error || '获取成员信息失败')
  } finally {
    loading.value = false
...
const assignRole = async (member) => {
  ...
  if (Number.isNaN(selectedRole)) {
    console.error('请选择有效职位')
    return
  }
  if (selectedRole === member.role) {
    console.error('该成员已是此职位')
    return
  }
  try {
    ...
    if (res.data.ok) {
      console.error('职位调整成功')
      fetchMembers()
    } else {
      console.error(res.data.error || '调整失败')
    }
  } catch (e) {
    console.error(e.response?.data?.error || '调整失败')
  }
}
...
const kickMember = async (member) => {
  ...
  try {
    ...
    if (res.data.ok) {
      console.error('已踢出该成员')
      fetchMembers()
    } else {
      console.error(res.data.error || '操作失败')
    }
  } catch (e) {
    console.error(e.response?.data?.error || '操作失败')
  }
}
