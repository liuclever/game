...
const storeBeast = async (beastId) => {
  try {
    ...
    if (res.data?.ok) {
      ...
      console.error(res.data?.error || '寄存失败')
    }
  } catch (err) {
    console.error('store beast failed', err)
    console.error(err.response?.data?.error || '寄存失败')
  }
}
...
const retrieveBeast = async (storageId) => {
  try {
    ...
    if (res.data?.ok) {
      ...
      console.error(res.data?.error || '取出失败')
    }
  } catch (err) {
    console.error('retrieve beast failed', err)
    console.error(err.response?.data?.error || '取出失败')
  }
}
