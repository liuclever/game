...
const doReset = async () => {
  ...
  try {
    ...
    console.error(e?.response?.data?.error || '重置失败')
  }
}
...
const doChallenge = async () => {
  ...
  try {
    ...
    console.error(e?.response?.data?.error || '挑战失败')
  }
}
...
const viewReport = async (stage) => {
  ...
  try {
    ...
    if (res.data?.ok) {
      ...
      console.error(res.data?.error || '暂无战报')
    }
  } catch (e) {
    console.error(e?.response?.data?.error || '暂无战报')
  }
}
