...
const doReset = async () => {
  ...
  try {
    ...
    showMessage(e?.response?.data?.error || '重置失败', 'error')
  }
}
...
const viewReport = async (stage) => {
  ...
  try {
    ...
    if (res.data?.ok) {
      ...
      showMessage(res.data?.error || '暂无战报', 'error')
    }
  } catch (e) {
    showMessage(e?.response?.data?.error || '暂无战报', 'error')
  }
}
...
const claimReward = async (stage) => {
  ...
  try {
    ...
    showMessage(e?.response?.data?.error || '领取失败', 'error')
  }
}
