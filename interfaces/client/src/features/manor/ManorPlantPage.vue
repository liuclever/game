...
const confirmPlant = async () => {
  ...
  try {
    ...
    const data = await res.json();
    if (data.ok) {
      showMessage(data.message || '种植成功', 'success');
      router.push('/manor');  // 返回庄园页面
    } else {
      showMessage(data.error || data.message || '种植失败', 'error');
    }
  } catch (error) {
...
const handleQuickPlant = async (seed: any) => {
  ...
  try {
    ...
    const data = await res.json();
    if (data.ok) {
      showMessage(data.message || '一键种植成功', 'success');
      router.push('/manor');  // 返回庄园页面
    } else {
      showMessage(data.error || data.message || '一键种植失败', 'error');
    }
  } catch (error) {
