...
const confirmPlant = async () => {
  ...
  try {
    ...
    const data = await res.json();
    if (data.ok) {
      console.error(data.message || '种植成功');
      router.push('/manor');  // 返回庄园页面
    } else {
      console.error(data.error || data.message || '种植失败');
    }
  } catch (error) {
...
const handleQuickPlant = async (seed: any) => {
  ...
  try {
    ...
    const data = await res.json();
    if (data.ok) {
      console.error(data.message || '一键种植成功');
      router.push('/manor');  // 返回庄园页面
    } else {
      console.error(data.error || data.message || '一键种植失败');
    }
  } catch (error) {
