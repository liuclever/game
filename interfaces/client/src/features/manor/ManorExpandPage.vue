...
const handleExpand = async () => {
  if (!isSpecialLand.value && nextLandIndex.value >= 10) {
    showMessage('普通土地最多10块，无法继续扩建', 'error');
    return;
  }
  if (!levelSatisfied.value) {
    showMessage(`等级不足，需要${toChineseStar(requiredStar.value, 'error')}星召唤师（${requiredLevel.value}级）`);
    return;
  }
  if (!itemSatisfied.value) {
    showMessage(`道具不足，需要庄园建造手册×${requiredManuals.value}`, 'error');
    return;
  }
  if (isSpecialLand.value && !vipSatisfied.value) {
    showMessage(`VIP等级不足，需要VIP ${requiredVip.value}`, 'error');
    return;
  }
  ...
  try {
    ...
    if (data.ok) {
      showMessage('扩建成功！', 'success');
      router.push('/manor');
    } else {
      showMessage(data.message || '扩建失败', 'error');
    }
  } catch (error) {
