...
const startCultivation = async (dungeonName) => {
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '开始修行失败')
    }
  } catch (e) {
    console.error('开始修行失败')
    console.error(e)
  }
}
...
const stopCultivation = async () => {
  try {
    ...
    if (res.data.ok) {
      console.error(res.data.message || '已终止修行')
      await fetchStatus()
      await fetchAvailableDungeons()
    } else {
      console.error(res.data.error || '终止修行失败')
      await fetchStatus()
    }
  } catch (e) {
    console.error('终止修行失败')
    console.error(e)
  }
}
