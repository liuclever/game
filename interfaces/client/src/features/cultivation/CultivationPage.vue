...
const startCultivation = async (dungeonName) => {
  try {
    ...
    if (res.data.ok) {
      ...
      showMessage(`${res.data.message}\nrequested=${requested}h, applied=${applied}h, vip=${vip}`, 'info')
      await fetchStatus()
    } else {
      showMessage(res.data.error || '开始修行失败', 'error')
    }
  } catch (e) {
    showMessage('开始修行失败', 'error')
    console.error(e)
  }
}
...
const endCultivation = async () => {
  try {
    ...
    if (res.data.ok) {
      ...
      showMessage(res.data.error || '结束修行失败', 'error')
      await fetchStatus()
    }
  } catch (e) {
    showMessage('结束修行失败', 'error')
    console.error(e)
  }
}
...
const stopCultivation = async () => {
  try {
    ...
    if (res.data.ok) {
      showMessage(res.data.message || '已终止修行', 'success')
      await fetchStatus()
      await fetchAvailableDungeons()
    } else {
      showMessage(res.data.error || '终止修行失败', 'error')
      await fetchStatus()
    }
  } catch (e) {
    showMessage('终止修行失败', 'error')
    console.error(e)
  }
}
