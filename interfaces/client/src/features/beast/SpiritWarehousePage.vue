...
const sellSpirit = async (spirit) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      showMessage(`出售成功，获得 ${res.data.spiritPower || 0} 灵力`, 'success')
    } else {
      showMessage(res.data.error || '出售失败', 'error')
    }
  } catch (err) {
    showMessage('网络错误，请稍后重试', 'error')
...
const sellAllUnlocked = async () => {
  ...
  if (unlockedSpirits.length === 0) {
    showMessage('没有可出售的战灵（未锁定的）', 'info')
    return
  }
  ...
  // 重新加载数据
  await loadData()
  showMessage(`成功出售 ${successCount} 个战灵，获得 ${totalPower} 灵力`, 'success')
}
