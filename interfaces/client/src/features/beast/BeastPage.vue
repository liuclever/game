...
const retrieveFromStorage = async (record) => {
  try {
    ...
    if (res.data.ok) {
      showMessage('幻兽已取回', 'success')
      await loadBeastData()
      await loadStorageList()
    } else {
      showMessage(res.data.error || '取回失败', 'error')
    }
  } catch (err) {
