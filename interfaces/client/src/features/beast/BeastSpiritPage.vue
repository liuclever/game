...
const unequipSpirit = async (elementKey) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      showMessage('卸下成功，战灵已存入灵件室', 'success')
    } else {
      showMessage(res.data.error || '卸下失败', 'error')
    }
  } catch (err) {
...
const showIntro = () => {
  showMessage('战灵简介：战灵可以提升幻兽的百分比属性加成，每只幻兽可装备6个元素位置的战灵（土、火、水、木、金、神）。战灵需要与幻兽种族匹配才能装备。', 'info')
}

const viewWarehouse = () => {
  if ((playerLevel.value || 0) < 35) {
    showMessage('35级才能解锁', 'info')
    return
  }
