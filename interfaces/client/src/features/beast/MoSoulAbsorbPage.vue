...
const performBatchAbsorb = async () => {
  if (isMaxLevel.value) {
    console.error('目标魔魂已满级，无法继续升级')
    return
  }

  if (!targetMosoul.value) {
    console.error('目标魔魂不存在')
    return
  }

  if (!mosouls.value || mosouls.value.length === 0) {
    console.error('没有可选的魔魂材料')
    return
  }
  ...
  try {
    ...
    if (res.data.ok) {
      console.error(res.data.message || '一键噬魂成功')
      await loadInitialData()
    } else {
      console.error(res.data.error || '一键噬魂失败')
    }
  } catch (err) {
...
const performAbsorb = async (materialMosoul) => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '摄魂失败')
    }
  } catch (err) {
    console.error('网络错误，请稍后重试')
