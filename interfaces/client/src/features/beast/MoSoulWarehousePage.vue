...
const performBatchAbsorbFromWarehouse = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      showMessage(res.data.message || '一键噬魂成功', 'success')
      await loadData()
    } else {
      showMessage(res.data.error || '一键噬魂失败', 'error')
    }
  } catch (err) {
...
const lockMoSoul = (mosoul) => {
  showMessage(`锁定【${mosoul.name}】功能待实现`, 'info')
}
