...
const doHunting = async (hunter) => {
  if (!hunter.available) {
    console.error(`${hunter.name}尚未解锁`)
    return
  }

  if (hunter.cost_type === 'fabao') {
    if (soulCharm.value < hunter.cost) {
      console.error('追魂法宝不足')
      return
    }
  } else {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '猎魂失败')
    }
  } catch (err) {
...
const doOneKeyHunting = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '一键猎魂失败')
    }
  } catch (err) {
    console.error('网络错误，请稍后重试')
...
<template>
  <div class="hunting-page">
    ...
    <div class="section title-section">
      【魔魂-猎魂】 <a class="link" @click="() => window.console.error('猎魂简介：花费铜钱让猎魂师帮你捕捉魔魂，不同猎魂师获得高品质魔魂的概率不同。')">简介</a>
    </div>
