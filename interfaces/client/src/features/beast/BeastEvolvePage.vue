...
const confirmEvolve = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      showMessage(`进化成功！【${beast.value.name}】已进化至${res.data.newRealm}`, 'success')
      // 直接跳转回幻兽详情页，让详情页重新加载最新数据
      router.push(`/beast/${beastId}`)
    } else {
      showMessage(res.data.error || '进化失败', 'error')
    }
  } catch (err) {
    console.error('进化失败:', err)
    showMessage('进化失败，请稍后重试', 'error')
  }
}
