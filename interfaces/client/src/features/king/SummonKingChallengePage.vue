...
const loadKingInfo = async () => {
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      summonKing.value = res.data.summonKing || null  // 获取召唤之王信息
...
const doChallenge = async (target) => {
  ...
  if (todayCount.value >= todayMax.value) {
    console.error('今日挑战次数已用完')
    return
  }
  if (cooldownRemaining.value > 0) {
    console.log(`冷却中，还需等待${cooldownRemaining.value}秒`)
    return
  }
  ...
  try {
    ...
    if (res.data.ok) {
      ...
      console.error(res.data.error || '挑战失败')
      loadKingInfo()
      loadDynamics()
    }
  } catch (e) {
...
<template>
  <div class="king-page">
    ...
    <div class="section">
      召唤之王：
      <template v-if="summonKing">
        <a class="link blue" @click="viewPlayer(summonKing.userId)">{{ summonKing.nickname }}</a>
      </template>
      <template v-else>
