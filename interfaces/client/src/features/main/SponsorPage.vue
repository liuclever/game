...
const submitOrder = async () => {
  ...
  try {
    ...
    if (res.data?.ok && res.data.pay_url) {
      ...
      showMessage(res.data?.error || '创建订单失败', 'error')
    }
  } catch (err) {
    console.error('创建订单失败', err)
    showMessage('创建订单失败，请稍后再试', 'error')
  } finally {
    paying.value = false
  }
}
...
const handleExchange = async () => {
  ...
  const amount = Number(exchangeAmount.value)
  if (!amount || amount <= 0) {
    showMessage('请输入有效的宝石数量', 'info')
    return
  }
  if (amount > sponsorInfo.value.silverDiamond) {
    showMessage('宝石不足', 'error')
    return
  }
...
onMounted(() => {
  if (route.query.result === 'success') {
    showMessage('支付完成，宝石将在几秒内到账', 'info')
  }
  loadSponsorInfo()
})
</script>
