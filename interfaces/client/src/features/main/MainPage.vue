...
const stopCultivation = async () => {
  ...
  try {
    ...
    console.error(e.response?.data?.error || '终止修行失败')
  } finally {
    stopping.value = false
  }
}
...
const handleSignin = async () => {
  ...
  try {
    ...
    console.error(e?.response?.data?.error || '签到失败')
  }
}
...
<template>
  <div class="main-page">
    ...
    <div class="section">
      <a class="link" @click="handleLink('联盟')">联盟</a>. <a class="link" @click="handleLink('盟战')">盟战</a>. <a class="link" @click="goMap">地图</a>. <a class="link" @click="handleLink('天赋')">天赋</a>. <a class="link" @click="handleLink('化仙')">化仙</a>
    </div>
    <div class="section">
