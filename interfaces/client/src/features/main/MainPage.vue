...
const stopCultivation = async () => {
  ...
  try {
    ...
    showMessage(e.response?.data?.error || '终止修行失败', 'error')
  } finally {
    stopping.value = false
  }
}
...
const handleSignin = async () => {
  ...
  try {
    ...
    if (res.data?.ok) {
      ...
      showMessage(res.data?.error || '签到失败', 'error')
    }
  } catch (e) {
    showMessage(e?.response?.data?.error || '签到失败', 'error')
  }
}
...
const goBone = async () => {
  ...
  // 若无幻兽或请求失败，提示用户
  showMessage('您暂无幻兽，无法查看战骨', 'error')
}
