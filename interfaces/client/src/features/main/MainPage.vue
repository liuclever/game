...
const loadSigninStatus = async () => {
  signinStatus.value.loading = true
  try {
    const res = await http.get('/signin/info')
    if (res.data.ok) {
      signinStatus.value.hasSigned = res.data.hasSigned || false
    }
  } catch (e) {
    console.error('加载签到状态失败', e)
  } finally {
    signinStatus.value.loading = false
  }
}
...
<template>
  <div class="main-page">
    ...
    <div class="section">
      今日 <template v-if="signinStatus.hasSigned"><span class="bold">已签到</span></template><a v-else class="link" @click="handleLink('签到')">签到</a>
    </div>
    <div class="section">
    ...
    <div class="section">
      <a class="link" @click="handleLink('好友')">切磋</a>. <a class="link" @click="goTower">闯塔</a>. <a class="link" @click="handleLink('战场')">战场</a>. <a class="link" @click="handleLink('擂台')">擂台</a>. <span class="link readonly">坐骑</span>
    </div>
    <div class="section">
    ...
    <div class="section">
      <a class="link" @click="handleLink('兑换')">兑换</a>. <a class="link" @click="handleLink('签到')">签到</a>. <span class="link readonly">论坛</span>. <a class="link" @click="handleLink('VIP')">VIP</a>. <span class="link readonly">安全锁</span>
    </div>
