...
const claimWelcomeGift = async (level) => {
  try {
    ...
    if (res.data?.ok) {
      showMessage(res.data.message || `成功领取VIP${level}见面礼包！`, 'success')
      playerInfo.value.claimedGiftLevels.push(level)
    } else {
      showMessage(res.data?.error || '领取失败', 'error')
    }
  } catch (error) {
    showMessage('领取失败: ' + (error.response?.data?.error || error.message, 'error'))
  }
}
...
const claimDailyCopper = async () => {
  try {
    ...
    if (res.data?.ok) {
      showMessage(res.data.message || '领取成功！', 'success')
