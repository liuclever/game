...
def tower_auto_challenge():
    ...
    tower_type = data.get("tower_type", "tongtian")
    use_buff = data.get("use_buff", True)
    is_continue = data.get("is_continue", False)  # 新增：是否是继续挑战
...
def get_zhenyao_info():
    ...
    if services.zhenyao_service.inventory_service:
        zhenyao_fu_count = services.zhenyao_service.inventory_service.get_item_count(user_id, ZHENYAO_FU_ITEM_ID)
