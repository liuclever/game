...
def get_king_info():
    ...
    if king_rows:
        summon_king = {
            "userId": king_rows[0]['user_id'],
            "nickname": king_rows[0]['nickname'] or f"玩家{king_rows[0]['user_id']}"
        }
    ...
    return jsonify({
        ...
        "summonKing": summon_king,  # 当前召唤之王信息
    })
...
def king_challenge():
    ...
    # 检查等级限制（20级）
    player_check = services.player_repo.get_by_id(user_id)
    if not player_check or player_check.level < 20:
        return jsonify({"ok": False, "error": "需要达到20级才能参加召唤之王挑战赛"})
    
    # 检查预选赛时间（周二、周三、周四可以挑战）
    today = datetime.now()
    weekday = today.weekday()  # 0=周一, 1=周二, ..., 6=周日
    
    if weekday == 0:
        return jsonify({"ok": False, "error": "周一为报名日，预选赛将于周二0点开始"})
    elif weekday == 4:  # 周五
...
def king_register():
    ...
    return jsonify({"ok": True, "message": "报名成功！预选赛将于周二0点开始"})
...
def use_battle_pass():
    ...
    BATTLE_PASS_ITEM_ID = 8001  # 召唤之王战令
    ...
    try:
        ...
        inventory_items = execute_query(
            ...
            (user_id, BATTLE_PASS_ITEM_ID)
        )
        
        if not inventory_items or inventory_items[0]['quantity'] < 1:
            return jsonify({"ok": False, "error": "没有召唤之王战令道具"})
        
        # 扣除战令
        services.inventory_service.remove_item(user_id, BATTLE_PASS_ITEM_ID, 1)
        
        # 增加挑战次数（减少已用次数，但不能低于0）
        execute_update(
            """UPDATE king_challenge_rank 
...
def claim_king_reward():
    """领取正赛奖励（需在本周内领取，下周一0点后无法领取）"""
    user_id = get_current_user_id()
    if not user_id:
        return jsonify({"ok": False, "error": "请先登录"})
    
    # 检查领取时间（周一0点后无法领取上周奖励）
    today = datetime.now()
