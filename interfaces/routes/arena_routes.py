...
def get_arena_dynamics():
    ...
    # 获取总共50条记录（5页）
    total_limit = page_size * max_pages
    
    if dynamic_type == "personal" and user_id:
        all_logs = ARENA_BATTLE_REPO.get_user_battles(user_id, total_limit)
    else:
        all_logs = ARENA_BATTLE_REPO.get_recent_battles(arena_type=arena_type, limit=total_limit)
